<script lang="ts">
    ...
    import { users, me } from "$lib/accountability/stores"
    import { usersToUserChoices } from "$lib/accountability/helpers"
    import { showProjectModal, projectModalData, createProject, updateProject, deleteProject, refreshProjects } from "$lib/accountability/projects"
    ...
    async function save() {
        ...
        if ($projectModalData.action == "create") {
            ...
            if (res.ok) {
                if ($page.params.project == $projectModalData.project.id) goto (`/accountability/deals/0/`)
                $showProjectModal = false
                try {
                    await refreshProjects()
                } catch (error) {
...
<Modal bind:open={$showProjectModal} {title} large={true} on:click={save} bind:disabled={disabledModal}>

    {#if $projectModalData.action == 'delete'}
        <p class="text-center text-sm text-a-error-500">
            Do you really wish to delete project {$projectModalData.project?.name}? 
            This will delete the project for all users. 
    ...
    <div class="grid grid-cols-2 w-full divide-x">
        ...
        <div class="pr-4 lg:pr-14">
            ...
            <Input type="text" label="Name" placeholder="Add name" bind:value={form.name} status={formStatus.name} message={formErrors.name} disabled={disabledContent} />
            <Input type="textarea" label="Description" placeholder="Add description" bind:value={form.description} disabled={disabledContent}
                   maxlength={280} status={formStatus.description} message={formErrors.description} />
            <Input type="multiselect" label="Users (who can edit)" placeholder="Search for user" disabled={disabledContent}
                   choices={userChoices} bind:value={form.editors} badgeType="avatar" />
            <Avatar initials={$me.initials} label="{$me.name} (owner)" extraClass="mt-1" padding={true} disabled={disabledContent} />
            <Divider />
            <Card>
