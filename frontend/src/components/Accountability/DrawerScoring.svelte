<script lang="ts">
  ...
  let openNavigationConfirmationModal = $state(false)
  ...
  let variableInfo = $derived(
    vggtVariables.filter(v => v.number == $currentVariable)[0],
  )
  let deal = $derived($deals.find(deal => deal.id == $currentDeal) ?? undefined)
  ...
  let status: string = $state("TO_SCORE")
  let assignee: number | undefined = $state(undefined)
  ...
  let previousAvailable = $derived(isPreviousAvailable($currentVariable))
  let nextAvailable = $derived(isNextAvailable($currentVariable))
  ...
  <div class="flex h-screen flex-col divide-y divide-a-gray-200">
    ...
    <div class="p-6">
      <div class="flex justify-between">
        ...
        <button
          ...
          onclick={() => navigationConfirmationModal("quit")}
        >
          <IconXMark size="24" />
        </button>
      </div>
    ...
    <div class="flex grow-0 justify-between p-6">
      <Button
        ...
        type="outline"
        style="neutral"
        onclick={() => navigationConfirmationModal("previous")}
      ...
      <div class="flex gap-4">
        <Button
          ...
          type="outline"
          style="neutral"
          onclick={() => navigationConfirmationModal("next")}
