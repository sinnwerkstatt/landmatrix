<script lang="ts">
    ...
    function joinArrays(choices, categories) {
        ...
        categories.forEach(category => {
            ...
            let obj = { label: category.label, choices: categoryChoices, values: categoryChoices.map(e => e.value) }
        ...
        if (orphans.length > 0) {
            let obj = { label: orphansLabel, choices: orphans, values: orphans.map(e => e.value) }
            obj.checked = arrayIncludesAllOf(group, obj.values)
            obj.partiallyChecked = arrayIncludesAnyOf(group, obj.values)
            result = [...result, obj]
        }
