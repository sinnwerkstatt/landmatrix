<template>
  <div class="container" id="graph-div">
    ...
    <div class="row">
      <div id="produce-info" class="col-md-12" style="min-height: 400px; height: 70vh">
      </div>
    </div>
  </div>
</template>
...
function buildTreeChart(treeData) {
  ...
  leaf.append("text")
    ...
    .selectAll("tspan")
    .data(d => d.data.name.split(/(?=[A-Z][a-z])|\s+/g).concat(format(d.data.value)+' ha'))
    .join("tspan")
    .attr("x", 3)
          ...
          for (let field of fields) {
            ...
            for (let entry of deal[field]) {
              for (let key of entry.value) {
                ...
                areaTotals[field][key] = (areaTotals[field][key] + dealSize) || dealSize;
              }
            }
          }
        }
