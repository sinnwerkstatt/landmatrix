...
class TransnationalDealsQuerySet(FakeQuerySet):
    ...
    QUERY = """
SELECT DISTINCT
    CONCAT(deal_country.fk_region_id, '.', deal_country.name, '#!#', deal_country.id) AS target_country,
    ARRAY_AGG(DISTINCT CONCAT(investor_country.fk_region_id, '.', investor_country.name,  '#!#', investor_country.id)) AS investor_country
