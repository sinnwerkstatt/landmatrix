...
class TransnationalDealsByTargetCountryQuerySet(FakeQuerySet):
...
(
    SELECT DISTINCT
        a.id,
        deal_region.id as region_id,
        deal_region.name as region
    FROM landmatrix_activity                       AS a
    ...
    WHERE
        ...
        AND pi_st.name IN ('active', 'overwritten')
                 ...
                 AND deal_region.name IS NOT NULL
        %s
)                                           AS sub
WHERE sub.id = a.id
...
"""
    ...
    def all(self):
        country_sql = " AND investor_country.id = %s " % self.country if self.country else ''
        self._filter_sql += country_sql
        return FakeQuerySet.all(self)


class TransnationalDealsByInvestorCountryQuerySet(TransnationalDealsByTargetCountryQuerySet):
...
(
    SELECT DISTINCT
        a.id,
        investor_region.id as region_id,
        investor_region.name as region
    FROM landmatrix_activity                       AS a
    JOIN      landmatrix_status                                        ON landmatrix_status.id = a.fk_status_id
    LEFT JOIN landmatrix_involvement               AS i                ON i.fk_activity_id = a.id
    LEFT JOIN landmatrix_stakeholder               AS s                ON i.fk_stakeholder_id = s.id
    ...
    WHERE
        ...
        AND pi_st.name IN ('active', 'overwritten')
                 AND investor_country.id <> deal_country.id
                 AND investor_country.name IS NOT NULL
        %s
)                                           AS sub
WHERE sub.id = a.id
GROUP BY sub.region_id, sub.region
"""
    ...
    def all(self):
        country_sql = " AND deal_country.id = %s " % self.country if self.country else ''
        self._filter_sql += country_sql
        return FakeQuerySet.all(self)
