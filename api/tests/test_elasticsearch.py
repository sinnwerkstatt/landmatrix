...
class APIElasticsearchTestCase(TestCase):
    ...
    required_properties = {
        'deal': ['id', 'activity_identifier', 'historical_activity_id', 'history_date', 'status',
                 'is_public', 'deal_scope', 'deal_size', 'current_contract_size', 'current_production_size',
                 'current_negotiation_status', 'current_implementation_status', 'init_date', 'top_investors'],
        'location': ['id', 'activity_identifier', 'historical_activity_id', 'history_date', 'status',
                 'is_public', 'deal_scope', 'deal_size', 'current_contract_size', 'current_production_size',
                 'current_negotiation_status', 'current_implementation_status', 'init_date', 'top_investors'],
        'data_source': [],
        'contract': [],
        'involvement_size': ['deal_id', 'activity_identifier', 'target_country', 'target_country_display',
                             'target_region', 'target_region_display', 'deal_size', 'deal_scope'],
        'investor': ['id', 'investor_identifier', 'top_investors', 'deal_count', 'roles', 'parent_company_of',
    ...
    def test_create_index(self):
        def index_exists():
            try:
                response = self.elasticsearch.conn.send_request('HEAD',
    ...
    def count_documents(self, doc_type, query):
        response = self.elasticsearch.conn.search({'query': {'term': query}},
    ...
    def assert_activity_indexed(self, activity_identifier):
        ...
        for doc_type in ('deal', 'location'):
            self.assertGreater(self.count_documents(doc_type, {'activity_identifier': activity_identifier}), 0,
                               msg=f"No documents for doc type '{doc_type}'")

    def assert_activity_not_indexed(self, activity_identifier):
        # FIXME: Check all DOC_TYPES_ACTIVITY in the future
        for doc_type in ('deal', 'location'):
            self.assertEqual(0, self.count_documents(doc_type, {'activity_identifier': activity_identifier}),
    ...
    def assert_investor_indexed(self, investor_identifier):
        ...
        for doc_type in ('investor', ):
            self.assertGreater(self.count_documents(doc_type, {'investor_identifier': investor_identifier}), 0,
                               msg=f"No documents for doc type '{doc_type}'")

    def assert_investor_not_indexed(self, investor_identifier):
        # FIXME: Check all DOC_TYPES_INVESTOR in the future
        for doc_type in ('investor', ):
            self.assertEqual(0, self.count_documents(doc_type, {'investor_identifier': investor_identifier}),
                             msg=f"Indexed documents for doc type '{doc_type}'")

    def test_index_activity_with_activity_identifier(self):
        activity = HistoricalActivity.objects.get(id=10)
        self.elasticsearch.index_activity(activity_identifier=activity.activity_identifier)
    ...
    def test_index_investor_with_investor_identifier(self):
        ...
        self.elasticsearch.index_investor(investor_identifier=investor.investor_identifier)
        self.elasticsearch.refresh_index()
    ...
    def test_get_activity_documents(self):
        ...
        # FIXME: Check all DOC_TYPES_ACTIVITY in the future
        for doc_type in ('deal', 'location'):
            docs = self.elasticsearch.get_activity_documents(activity, doc_type=doc_type)
            props = self.required_properties.get(doc_type)
            for doc in docs:
                for prop in props:
                    self.assertIn(prop, doc.keys(),
    ...
    def test_get_display_properties(self):
        properties = {
            'deal': {
                'current_negotiation_status': ['Under negotiation', 'Intended (Under negotiation)'],
                'current_implementation_status': ['Project not started', 'Project not started'],
                'is_public': ['True', 'Yes'],
            ...
            'investor': {
                'fk_country': ['104', 'Myanmar'],
            },
            'involvement': {}
        }
        for doc_type, props in properties.items():
            doc = dict((k, v[0]) for k, v in props.items())
            display_props = self.elasticsearch.get_display_properties(doc, doc_type=doc_type)
            for key, value in props.items():
                self.assertEqual(value[1], display_props.get(f'{key}_display'),
    ...
    def test_get_investor_documents(self):
        ...
        # FIXME: Check all DOC_TYPES_INVESTOR in the future
        for doc_type in ('investor', ):
            docs = self.elasticsearch.get_investor_documents(investor, doc_type=doc_type)
            props = self.required_properties.get(doc_type)
            for doc in docs:
                for prop in props:
                    self.assertIn(prop, doc.keys(),
    ...
    def test_search(self):
        ...
        self.elasticsearch.refresh_index()
        results = self.elasticsearch.search(query={'terms': {'activity_identifier': [1, 2]}},
                                            doc_type='deal',
    ...
    def test_aggregate(self):
        ...
        results = self.elasticsearch.search(query={'terms': {'activity_identifier': [1, 2]}},
                                            aggs=aggregations,
                                            doc_type='deal')
        activity_identifiers = results.get('activity_identifiers', {}).get('buckets', [])
