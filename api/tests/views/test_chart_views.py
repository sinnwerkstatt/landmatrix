...
class ChartViewsTestCase(TestCase):
    ...
    def test_logging_view(self):
        ...
        self.assertEqual(200, response.status_code)
        response_dict = dict((d['name'], d) for d in response.data)
        self.assertEqual(2, response_dict.get('Contract signed', {}).get('deals'))
    ...
    def test_investment_intention_list_view(self):
        ...
        self.assertEqual(200, response.status_code)
        response_dict = dict((d['name'], d) for d in response.data)
        self.assertEqual(1, response_dict.get('Forest logging / management', {}).get('deals'))
        ...
        self.assertEqual(1, response_dict.get('Mining', {}).get('deals'))
        self.assertEqual(1000, response_dict.get('Mining', {}).get('hectares'))
