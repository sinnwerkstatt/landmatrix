...
class FilterCreateViewTestCase(TestCase):
    ...
    def test_filter_with_investor(self):
        data = {
            'variable': 'investor_identifier',
            'operator': 'is',
            'value': '1',
        }
        response = self.client.post(reverse('api_filter_create', kwargs={'doc_type': 'investor'}), data=data)
        self.assertEqual(200, response.status_code)
    ...
    def test_preset_with_duplicates(self):
        custom_preset = {'name': 'custom_preset', 'preset_id': '1', 'label': 'Exclude Mining', 'hidden': False}
        session = self.client.session
...
class FilterPresetViewTestCase(TestCase):
    ...
    def test_with_group(self):
        ...
        response = self.client.get(reverse('api_filter_preset', kwargs={'doc_type': 'deal'}), data)
        self.assertEqual(200, response.status_code)
        response_dict = dict((d['name'], d) for d in response.data)
