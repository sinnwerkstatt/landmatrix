...
from .map_stakeholder_investor import MapStakeholderInvestor, get_stakeholder_id
    ...
    def map_record(cls, record, save=False, verbose=False):
        ...
        try:
            investor = landmatrix.models.Investor.objects.using(V2).get(
                pk=get_stakeholder_id(sh_tag_group.fk_stakeholder_id)
            )
            ...
            pass
        try:
            hinvestor = landmatrix.models.HistoricalInvestor.objects.using(V2).get(
                pk=get_stakeholder_id(sh_tag_group.fk_stakeholder_id)
            )
            hinvestor.comment = record['comment']
            if save:
                hinvestor.save(using=V2)
        except landmatrix.models.HistoricalInvestor.DoesNotExist:
            raise
