...
from apps.editor.views import ApproveActivityDeleteView, LogDeletedView, ManageDeletesView, ManageForUserView
...
class TestDealDelete(BaseDealTestCase):
    ...
    def test_reporter(self):
        ...
        response = DealDeleteView.as_view()(request, deal_id=activity.activity_identifier)
        #if response.status_code == 200:
        #    # For debugging purposes
        #    errors = list(filter(None, [form.errors or None for form in response.context_data['forms']]))
        #    self.assertEqual([], errors)
        self.assertEqual(302, response.status_code, msg='Delete deal by Reporter does not redirect')
