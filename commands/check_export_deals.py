...
class Command(BaseCommand):
    help = (
        "Check if export number of deals is same as grid."
    ...
    def handle(self, *args, **options):
        ...
        results = {}
        deals = export.execute_elasticsearch_query(
        ...
        self.stdout.write(
            "{} deals missing in grid ({})".format(
                str(len(missing_grid)), ", ".join([str(i) for i in missing_grid])
            )
        )
        missing_export = grid_ids - export_ids
        self.stdout.write(
            "{} deals missing in export ({})".format(
                str(len(missing_export)), ", ".join([str(i) for i in missing_export])
            )
        )
