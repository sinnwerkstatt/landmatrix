...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i, row in enumerate(ws.rows):
            if i == 0:
                for column in row:
                    columns.append({"label": column.value, "items": []})
                continue
        ...
        for column in columns:
            ...
            if count == 0 and column["label"] not in EMPTY_ALLOWED:
                self.stderr.write(
                    'No values in column "%s" (Sheet: Deals)' % column["label"]
                )

            count = len(list(filter(None, column["items"])))
            if column["label"] in NO_EMPTY_ALLOWED and count < rows:
                self.stderr.write(
                    '%i missing values in column "%s" (Sheet: Deals)'
                    % (rows - count, column["label"])
                )
