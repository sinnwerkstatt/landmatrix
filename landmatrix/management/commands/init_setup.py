...
from wagtailcms.models import WagtailRootPage, CountryIndex, WagtailPage, CountryPage, RegionIndex, RegionPage

User = get_user_model()


def create_superuser(name, email, password, firstname, lastname, superuser=False):
    u_obj = User.objects
    try:
        if superuser:
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        Site.objects.get_or_create(hostname='localhost', port=8000, root_page=wt_root, is_default_site=True)
        ...
        asia = RegionPage(title='Asia', slug='asia', region=Region.objects.get(name='Asia'))
