...
from landmatrix.models import HistoricalInvestor, InvestorVentureInvolvement
...
class InvestorVentureInvolvementFormTestCase(TestCase):
    ...
    def test_get_display_properties(self):
        ...
        display_keys = {'fk_venture_display', 'fk_investor_display', 'role_display',
                        'investment_type_display', 'percentage_display', 'loans_amount_display',
                        'loans_currency_display', 'loans_date_display', 'comment_display',
                        'fk_status_display'}
...
class ParentCompanyFormTestCase(TestCase):
    ...
    def test_init(self):
        ...
        self.assertEqual({20}, set(investor_field.queryset.values_list('pk', flat=True)))
        self.assertEqual({'20'}, set(investor_field.widget.data.keys()))
        self.assertEqual({'investor-identifier': 2}, investor_field.widget.data.get('20'))
...
class BaseInvolvementFormSetTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.formset_class = modelformset_factory(HistoricalInvestorVentureInvolvement,
    ...
    def test_save(self):
        ...
        for involvement in involvements:
            self.assertEqual(HistoricalInvestor.STATUS_PENDING, involvement.fk_status_id)
        ...
        for involvement in deleted_involvements:
            self.assertEqual(HistoricalInvestor.STATUS_DELETED, involvement.fk_status_id)
