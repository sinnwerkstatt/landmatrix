...
class BaseDealTestCase(PermissionsTestCaseMixin,
    ...
    def assert_deal_not_in_list(self, response, activity):
        items = list(filter(lambda i: i['history_id'] == activity.id, response.context_data['items']))
        self.assertEqual(0, len(items), msg='Deal does appear in Manage section (but it should not)')
    ...
    def _get_investors(involvements):
        ...
        for involvement in involvements:
            ...
            if investor.venture_involvements.count() > 0:
                investors.update(BaseDealTestCase._get_investors(investor.venture_involvements.all()))
        return investors
    ...
    def _get_investor_status_list(involvements):
        ...
        :return:
        """
        return set([i.fk_status_id for i in BaseDealTestCase._get_investors(involvements)])
    ...
    def assert_investors_approved(self, activity):
        ...
        :return:
        """
        investor_status = BaseDealTestCase._get_investor_status_list(activity.involvements.all())
...
class BaseInvestorTestCase(BaseDealTestCase):
    ...
    def assert_investor_not_in_list(self, response, investor):
        items = list(filter(lambda i: i['history_id'] == investor.id, response.context_data['items']))
        self.assertEqual(0, len(items), msg='Investor does appear in Manage section (but it should not)')
