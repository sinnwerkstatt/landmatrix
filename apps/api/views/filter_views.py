...
from apps.grid.views.browse_filter_conditions import (
    get_activity_field_label,
    get_investor_field_label,
)
...
class FilterCreateView(FilterDocTypeMixin, APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if "preset" in request_data:
            ...
            for filter_name, stored_filter in stored_filters.items():
                if (
                    stored_filter.get("variable", "") == variable
        ...
        stored_filters = dict(
            (k.replace("default_", ""), v) for k, v in stored_filters.items()
        )
...
class FilterDeleteView(FilterDocTypeMixin, APIView):
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not name:
            name = "filter_%i" % (
                len(request.session.get("%s:filters" % self.doc_type, [])) + 1
            )
...
class SetDefaultFiltersView(FilterDocTypeMixin, APIView):
    """
...
class FilterListView(FilterDocTypeMixin, APIView):
    """
...
class FilterPresetView(FilterDocTypeMixin, ListAPIView):
    """
