...
class FiltersTestCase(TestCase):
    ...
    fixtures = ["filters", "countries_and_regions", "users_and_groups"]
    ...
    def test_load_statuses_from_url(self):
        status_tests = (
            (AnonymousUser(), (2, 3)),
            (User.objects.get(username="administrator"), (2, 3)),
            (User.objects.get(username="administrator-staff"), (1, 2, 3, 4, 5, 6)),
        )
    ...
    def test_get_list_element_by_key(self):
        ...
        for value_test in value_tests:
            result = get_list_element_by_key(
                value_test[0], key=value_test[1], value=value_test[2]
            )
    ...
    def test_remove_all_dict_keys_from_mixed_dict(self):
        ...
        for value_test in value_tests:
            result = value_test[0]
            remove_all_dict_keys_from_mixed_dict(result, key_name=value_test[1])
...
class FilterTestCase(TestCase):
    ...
    def test_invalid_operator(self):
        self.assertRaises(
            ValueError,
            lambda: Filter(
                variable="activity_identifier", operator="is_not", value="1"
            ),
        )

    def test_no_display_value(self):
        filter_obj = Filter(variable="activity_identifier", operator="is", value="1")
    ...
    def test_to_sql_format(self):
        ...
        self.assertEqual(
            list(formatted_filter.keys())[0], "activity_identifier__value__is"
        )
        self.assertEqual(list(formatted_filter.values())[0], "1")
        filter_obj["operator"] = "in"
        formatted_filter = filter_obj.to_sql_format()
        self.assertEqual(
            list(formatted_filter.keys())[0], "activity_identifier__value__in"
        )
        self.assertEqual(list(formatted_filter.values())[0], ["1"])
    ...
    def test_parse_value(self):
        ...
        parsed_value = filter_obj.parse_value(
            "Myanmar", variable="target_country", key="value"
        )
