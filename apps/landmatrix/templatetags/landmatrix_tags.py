...
@register.simple_tag
def nav_regions():
    regions = RegionPage.objects.filter(live=True, region__isnull=False).order_by('title').values_list('region_id', 'slug', 'title')
    return regions
...
def nav_countries():
    ...
    other_countries = Country.objects.filter(is_target_country=True, high_income=False).exclude(
        id__in=[c.country.id for c in observatories if c.country]).only('id', 'slug', 'name').order_by('name')
