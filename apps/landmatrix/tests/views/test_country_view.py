...
class CountryViewTestCase(TestCase):
    ...
    def test_with_country_page(self):
        CountryPage.objects.create(
            title="Myanmar Page", slug="myanmar", country_id=104, path="/", depth=0
        )
        response = self.client.get(
            reverse("country", kwargs={"country_slug": "myanmar"})
        )
        self.assertEqual(200, response.status_code)

    def test_with_index_page(self):
        CountryIndex.objects.create(
            title="Country index", slug="country", path="/", depth=0
        )
        response = self.client.get(
            reverse("country", kwargs={"country_slug": "myanmar"})
        )
