...
from apps.landmatrix.pdfgen import PDFResponse, PDFViewMixin, update_querystring
...
class PDFResponseTestCase(TestCase):
    def test_init_with_filename(self):
        ...
        self.assertEqual(
            f'attachment; filename="{filename}"', response["Content-Disposition"]
        )
...
class PDFViewMixinTestCase(TestCase):
    ...
    def get_pdf_export_url(self):
        self.assertEqual(
            "/deal/1.pdf", self.mixin.get_pdf_export_url(self.request, deal_id=1)
        )

    def get_pdf_render_url(self):
        self.assertEqual(
            "/deal/1/", self.mixin.get_pdf_export_url(self.request, deal_id=1)
        )
    ...
    def test_render_url_to_pdf_response(self):
        with self.assertRaises(CalledProcessError):
            response = self.mixin.render_url_to_pdf_response(
