...
function buildPieChart() {
  ...
  d3.json("", function (data) {
    ...
    arcs
      .append("svg:text") //add percentage to each label
      .attr("transform", function (d) {
        //set the label's origin to the center of the arc
        //we have to make sure to set these before calling arc.centroid
        d.innerRadius = tr;
...
function buildDotChart() {
  ...
  d3.json("", function (data) {
    ...
    var xScale = d3.scale.linear().domain([x_start, x_end]).range([0, width]);
    ...
    for (var j = 0; j < data.length; j++) {
      ...
      var text = g.selectAll("text").data(data[j]["intentions"]).enter().append("text");

      var rScale = d3.scale
        .linear()
        .domain([
          0,
          d3.max(data[j]["intentions"], function (d) {
            return d[1];
          }),
        ])
        .range([2, 9]);
...
function buildAgriculturalPies() {
  ...
  $.get("/api/agricultural-produce.json", function (data) {
    ...
    (sum && $(".data-availability, .data").show()) ||
      $(".data-availability, .data").hide();
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      $("#pie-" + data[i]["region"])
        ...
        .find(".food-crop")
        .text(
          data[i]["agricultural_produce"]["food_crop"] +
            "% (" +
            numberWithCommas(data[i]["hectares"]["food_crop"]) +
            " ha)"
        );
      $("#pie-" + data[i]["region"])
        .parent()
        ...
        .find(".non-food-crop")
        .text(
          data[i]["agricultural_produce"]["non_food"] +
            "% (" +
            numberWithCommas(data[i]["hectares"]["non_food"]) +
            " ha)"
        );
      $("#pie-" + data[i]["region"])
        .parent()
        ...
        .find(".flex-crop")
        .text(
          data[i]["agricultural_produce"]["flex_crop"] +
            "% (" +
            numberWithCommas(data[i]["hectares"]["flex_crop"]) +
            " ha)"
        );
      $("#pie-" + data[i]["region"])
        .parent()
        ...
        .find(".multiple-crop")
        .text(
          data[i]["agricultural_produce"]["multiple_use"] +
            "% (" +
            numberWithCommas(data[i]["hectares"]["multiple_use"]) +
            " ha)"
        );
      var sum =
        data[i]["hectares"]["food_crop"] +
        data[i]["hectares"]["non_food"] +
        data[i]["hectares"]["flex_crop"] +
        data[i]["hectares"]["multiple_use"];
      $("#pie-" + data[i]["region"])
        .closest(".row")
        .prev()
        .find("h2 span")
        .text(numberWithCommas(sum) + " ha");
