...
class BaseGeometryWidgetTestCase(TestCase):
    ...
    def test_serialize(self):
        self.assertEqual(
            "POINT (0 1)", self.widget.serialize(GEOSGeometry("POINT(0 1)", srid=4326))
        )
    ...
    def test_get_context(self):
        context = self.widget.get_context(
            name="name", value=GEOSGeometry("POINT(0 1)", srid=4326)
        )
        keys = {
            "class",
            "geom_type",
    ...
    def test_get_context_with_different_srid(self):
        context = self.widget.get_context(
            name="name", value=GEOSGeometry("POINT(0 1)", srid=3857)
        )
...
class OpenLayersWidgetTestCase(TestCase):
    ...
    def test_serialize(self):
        ...
        self.assertEqual(
            expected, self.widget.serialize(GEOSGeometry("POINT(0 1)", srid=4326))
        )
