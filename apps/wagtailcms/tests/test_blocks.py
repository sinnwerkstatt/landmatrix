        ...
        context = block.get_context(
            {
                "title": "title",
                "faqs": [
                    {"slug": "slug1", "question": "question1", "answer": "answer1"},
                    {"slug": "slug2", "question": "question2", "answer": "answer2"},
                ],
            }
        )
...
class SliderBlockTestCase(TestCase):
    def test_get_context(self):
        ...
        context = block.get_context(
            {
                "title": "title",
                "images": [{"image": image, "url": "url", "caption": "caption"}],
            },
            {"request": request},
        )
        self.assertEqual("title", context.get("title"))
...
class TitleWithIconBlockTestCase(TestCase):
    ...
    def test_get_context(self):
        ...
        request.resolver_match = AttrDict(kwargs={"country_slug": "myanmar"})
        context = block.get_context(
            {"value": "value", "fa_icon": "fa_icon", "url": "url"}, {"request": request}
        )
...
class LatestDatabaseModificationsBlockTestCase(TestCase):
    ...
    def test_get_context(self):
        ...
        request = RequestFactory()
        request.resolver_match = AttrDict(kwargs={"country_slug": "myanmar"})
        context = block.get_context({"limit": "limit"}, {"request": request})
        self.assertEqual(104, context.get("country").id)
...
class TabsBlockTestCase(TestCase):
    def test_get_context(self):
        block = TabsBlock()
        context = block.get_context(
            {"tabs": [{"title": "title", "fa_icon": "fa_icon", "content": "content"}]}
        )
...
class FullWidthContainerBlockTestCase(TestCase):
    def test_get_context(self):
        ...
        context = block.get_context({"content": "content", "color": "color"})
