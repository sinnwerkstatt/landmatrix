...
from apps.grid.views.deal import DealCreateView, DealDeleteView, DealDetailView, DealListView, DealRecoverView, DealUpdateView
from apps.grid.views.export import ExportView

urlpatterns = [
    path('<int:deal_id>/', DealDetailView.as_view(), name='deal_detail'),
    path('<int:deal_id>.pdf', DealDetailView.as_view(), {'format': 'PDF'}, name='deal_detail_pdf'),
    ...
    path('<int:deal_id>/<int:history_id>/', DealDetailView.as_view(), name='deal_detail'),
    path('<int:deal_id>/<int:history_id>.pdf', DealDetailView.as_view(), {'format': 'PDF'}, name='deal_detail_pdf'),

    path('edit/<int:deal_id>/', DealUpdateView.as_view(), name='change_deal'),
    path('edit/<int:deal_id>/<int:history_id>/', DealUpdateView.as_view(), name='change_deal'),
