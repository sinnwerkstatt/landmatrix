...
class YearBasedWidgetTestCase(TestCase):
    def setUp(self):
        self.widget = YearBasedWidget(
            help_text="help_text", attrs={"attr_key": "attr_value"}
        )
...
class YearBasedSelectMultipleNumberTestCase(TestCase):
    def test(self):
        widget = YearBasedSelectMultipleNumber(
            choices=(), attrs={"placeholder": "placeholder"}
        )
...
class FileInputWithInitialTestCase(TestCase):
    def setUp(self):
        ...
        self.file = SimpleUploadedFile(
            self.file_name, b"", content_type="application/pdf"
        )
...
class InvestorSelectTestCase(TestCase):
    def test(self):
        choices = (("10", "Investor name #1"),)
        investor_identifiers = {"10": {"investor_identifier": "1"}}
        widget = InvestorSelect(choices=choices, data=investor_identifiers)
        output = widget.render("name", "10", attrs={})
        self.assertIn(
            '<option value="10" selected data-investor_identifier="1">', output
        )
