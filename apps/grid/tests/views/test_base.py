...
class TableGroupViewTestCase(ElasticSearchFixtureMixin, TestCase):
        ...
        self.group_results = [
            {
                "key": "Myanmar",
                "doc_count": 3,
                "all": {"buckets": [{"key": "Myanmar", "doc_count": 3}]},
                "target_region_display": {"buckets": [{"key": "Asia", "doc_count": 3}]},
                "deal_size": {"buckets": [{"key": 1000, "doc_count": 3}]},
                "intention": {"buckets": [{"key": "Mining", "doc_count": 2}]},
                "target_country": {"buckets": [{"key": "104", "doc_count": 3}]},
            }
        ]
    ...
    def test_get_group_value_query_with_country(self):
        ...
        expected = {
            "bool": {
                "filter": [{"bool": {"filter": {"term": {"target_country": 104}}}}]
            }
        }
    ...
    def test_get_group_value_query_with_region(self):
        ...
        expected = {
            "bool": {"filter": [{"bool": {"filter": {"term": {"target_region": 142}}}}]}
        }
        self.assertEqual(expected, self.view.get_group_value_query({}))
    ...
    def test_get_group_value_query_with_crop(self):
        ...
        expected = {"bool": {"filter": [{"bool": {"filter": {"term": {"crops": 1}}}}]}}
        self.assertEqual(expected, self.view.get_group_value_query({}))
    ...
    def test_get_group_value_query_with_investor(self):
        ...
        expected = {
            "bool": {
                "filter": [
                    {"bool": {"filter": {"term": {"investor_id": "Test Investor #1"}}}}
                ]
            }
        }
        self.assertEqual(expected, self.view.get_group_value_query({}))
    ...
    def test_get_group_aggs(self):
        ...
        expected = {"bool": {"must_not": [{"term": {"activity_identifier": ""}}]}}
    ...
    def test_get_columns_with_request(self):
        self.view.request.GET = QueryDict(
            "columns=activity_identifier&columns=deal_size"
        )
    ...
    def test_get_columns_with_group_value(self):
        ...
        self.view.group_value = "104"
        self.assertEqual(
            set(self.view.COLUMN_GROUPS["all"]), set(self.view.get_columns())
        )

    def test_get_columns_with_group(self):
        self.view.group = "target_country"
        self.assertEqual(
            set(self.view.COLUMN_GROUPS["target_country"]), set(self.view.get_columns())
        )
    ...
    def test_get_columns_dict_with_group(self):
        ...
        self.assertIsInstance(columns, OrderedDict)
        keys = set(
            k.replace("_display", "") for k in self.view.COLUMN_GROUPS["target_country"]
        )
        self.assertEqual(keys, set(columns.keys()))
        order_dict = {
    ...
    def test_get_deal_item(self):
        expected = OrderedDict(
            [
                ...
                (
                    "intention",
                    [{"value": "Mining", "slug": "Mining", "order_by": "Mining"}],
                ),
    ...
    def test_clean_intention_with_list(self):
        value = ["Mining", "Timber plantation", "Invalid"]
        expected = [
            {"value": "Mining", "slug": "Mining", "order_by": "Mining"},
            {
    ...
    def test_clean_investor_name_with_group_values(self):
        self.view.group_values = {
            "1": {"name": "Test investor #1", "fk_country_display": "Myanmar"}
        }
        value = {"value": "1"}
