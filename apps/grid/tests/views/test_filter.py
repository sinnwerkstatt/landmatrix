...
from apps.grid.views.filter import *
...
class FilterWidgetAjaxViewTestCase(TestCase):
    ...
    def get_result_dict(self, params, doc_type="deal"):
        request = self.factory.get(
            reverse("ajax_widget", kwargs={"doc_type": doc_type})
        )
    ...
    def test_with_activity_identifier(self):
        result = self.get_result_dict(
            "key_id=activity_identifier&name=value&operation=is&value=1"
        )
        self.assertEqual(
            ["lt", "gt", "gte", "lte", "is", "is_empty"],
            result.get("allowed_operations"),
        )
    ...
    def test_with_deal_count(self):
        result = self.get_result_dict(
            "key_id=deal_count&name=value&operation=is&value=1", doc_type="investor"
        )
        self.assertEqual(
            ["lt", "gt", "gte", "lte", "is", "is_empty"],
            result.get("allowed_operations"),
    ...
    def test_with_multi_value_field(self):
        result = self.get_result_dict(
            "key_id=intention&name=value&operation=is&value=1"
        )
        self.assertEqual(
            ["is", "not_in", "in", "is_empty"], result.get("allowed_operations")
        )
        self.assertIn(
            '<select name="value" id="id_value" class="valuefield form-control">',
            result.get("widget"),
        )

    def test_with_date_field(self):
        result = self.get_result_dict(
            "key_id=updated_date&name=value&operation=gte&value=2000-01-01"
        )
        self.assertEqual(
            ["lt", "gt", "gte", "lte", "is", "is_empty"],
            result.get("allowed_operations"),
    ...
    def test_without_boolean_field(self):
        result = self.get_result_dict(
            "key_id=file_not_public&name=value&operation=is&value="
        )
    ...
    def test_without_field(self):
        ...
        self.assertEqual(
            ["contains", "is", "is_empty"], result.get("allowed_operations")
        )
...
class FilterWidgetMixinTestCase(TestCase):
    ...
    fixtures = ["countries_and_regions", "users_and_groups", "filters"]
    ...
    def test_remove_country_region_filter(self):
        ...
        self.assertEqual(
            {"custom_filter"},
            set(self.mixin.request.session.get("deal:filters").keys()),
        )
    ...
    def test_set_default_filters_global(self):
        ...
        self.mixin.set_default_filters(
            {}, disabled_presets=[10, 4], enabled_presets=[1, 3, 4]
        )
        expected = {
            "default_preset_1",
            "default_preset_2",
