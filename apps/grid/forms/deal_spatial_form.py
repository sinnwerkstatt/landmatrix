...
from apps.ol3_widgets.widgets import LocationWidget, MapWidget
...
class DealSpatialForm(BaseForm):
    ...
    AREA_FIELDS = ("contract_area", "intended_area", "production_area")

    form_title = _("Location")
    tg_location = TitleField(required=False, label="", initial=_("Location"))
    level_of_accuracy = forms.ChoiceField(
        required=False, label=_("Spatial accuracy level"), choices=ACCURACY_CHOICES
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        area_attrs = {"bound_map_field_id": "{}-map".format(self["location"].html_name)}
    ...
    def get_location_map_widget_attrs(self):
        attrs = {"show_layer_switcher": True}
    ...
    def _clean_area_field(self, field_name):
        ...
        if value_is_file:
            ...
            shapefile_data = (
                hasattr(self.files, "getlist")
                and self.files.getlist(field_name)
                or self.files[field_name]
            )
            try:
    ...
    def get_data(cls, activity, group=None, prefix=""):
        ...
        for area_field_name in cls.AREA_FIELDS:
            area_attribute = activity.attributes.filter(
                fk_group__name=group, name=area_field_name
            ).first()
            if area_attribute:
...
class PublicDealSpatialForm(DealSpatialForm):
    ...
    location = forms.CharField(required=True, label=_("Location"), widget=MapWidget)
...
class DealSpatialBaseFormSet(BaseFormSet):
    ...
    def get_data(cls, activity, group=None, prefix=""):
        ...
        for i, group in enumerate(groups):
            form_data = DealSpatialForm.get_data(
