...
from apps.grid.fields import TitleField, YearBasedIntegerField
...
class DealEmploymentForm(BaseForm):
    ...
    tg_total_number_of_jobs_created_comment = forms.CharField(
        required=False, label=_("Comment on jobs created (total)"), widget=CommentInput
    )

    # Number of jobs for foreigners created
    tg_foreign_jobs_created = TitleField(
        required=False, label="", initial=_("Number of jobs for foreigners created")
    )
    ...
    foreign_jobs_current = YearBasedIntegerField(
        required=False, label=_("Current number of jobs (foreign)"), help_text=_("jobs")
    )
