...
class ParentCompanyForm(FieldsDisplayFormMixin, InvestorVentureInvolvementForm):
    ...
    def __init__(self, *args, **kwargs):
        ...
        valid_choice = self.data.get(
            "%s-fk_investor" % self.prefix, self.initial.get("fk_investor", None)
        )
        if valid_choice:
...
class BaseInvolvementFormSet(forms.BaseModelFormSet):
    ...
    def save(self, fk_venture, commit=True):
        ...
        for instance in instances:
            if (
                not hasattr(instance, "fk_investor") or not fk_venture
            ):  # pragma: no cover
                continue
            instance.id = None
...
ParentInvestorFormSet = forms.modelformset_factory(
    HistoricalInvestorVentureInvolvement,
    form=ParentInvestorForm,
    formset=BaseInvestorFormSet,
    extra=1,
    min_num=0,
    max_num=1,
    can_delete=True,
