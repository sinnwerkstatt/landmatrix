...
from apps.grid.views.browse_filter_conditions import get_investor_field_label
from apps.grid.views.filter import get_investor_variable_table
...
class InvestorListView(TableGroupView):
    ...
    GROUP_NAMES = {"fk_country": _("Country of registration/origin")}
    COLUMN_LABELS_MAP = {
    ...
    ORDER_MAP = {"investor_count": "_count"}
    ...
    def add_status_logic(self, query):
        ...
        query["filter"].append({"terms": {"fk_status": self.status_list}})
        return query
...
class InvestorFormsMixin:
    ...
    def form_invalid(self, investor_form, stakeholders_formset, investors_formset):
...
class InvestorCreateView(InvestorFormsMixin, CreateView):
    ...
    context_object_name = "investor"
    success_message = _(
        "The investor has been submitted successfully (#{}). It will be reviewed and published soon."
    )
    ...
    def form_valid(self, investor_form, stakeholders_formset, investors_formset):
        ...
        if "approve_btn" in self.request.POST and has_perm_approve_reject(
            ...
            messages.success(
                self.request, self.success_message.format(hinvestor.investor_identifier)
            )
...
class InvestorUpdateView(InvestorFormsMixin, UpdateView):
    ...
    model = HistoricalInvestor
    success_message = _(
        "Your changes to the investor have been submitted successfully. The changes will be reviewed and published soon."
    )
    success_message_admin = _(
        "Your changes to the investor have been saved successfully."
    )
...
class DeleteInvestorView(InvestorUpdateView):
    ...
    success_message = _(
        "The investor #{} has been marked for deletion. It will be reviewed and deleted soon."
    )
    ...
    def get(self, request, *args, **kwargs):
        ...
        return HttpResponseRedirect(
            reverse(
                "investor_detail", kwargs={"investor_id": hinvestor.investor_identifier}
            )
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not self.request.user.has_perm("landmatrix.change_historicalinvestor"):
            return HttpResponseRedirect(
                reverse(
                    "investor_detail",
                    kwargs={"investor_id": hinvestor.investor_identifier},
                )
            )
        # Create new historical activity
        hinvestor.pk = None
