...
from apps.grid.views.deal import get_forms
        ...
        if activity_2:
            ...
            deal_2 = (
                HistoricalActivity.objects.filter(
                    activity_identifier=deal_1.activity_identifier
                )
                .filter(history_date__lt=deal_1.history_date)
    ...
    if len(forms_1) != len(forms_2):  # pragma: no cover
        raise IndexError(
            ...
            % (
                deal_1.id,
                deal_2.id,
                deal_1.activity.history_id,
                deal_2.activity.history_id,
            )
        )
    comparison_forms = []
