...
class ExportView(FilterWidgetMixin, ElasticSearchMixin, View):
    ...
    def get_group_value_query(self, query):
        ...
        query["bool"]["filter"].append(
            {"bool": {"filter": {"term": {group: filter_value}}}}
        )
        return query
    ...
    def get(self, request, *args, **kwargs):
        ...
        results = {}
        # Search deals
        deals = self.execute_elasticsearch_query(
            query, doc_type="deal", fallback=False, sort=sort
        )
        ...
        if deal_id:
            ...
            query = {
                "ids": {
                    "type": "investor",
                    "values": get_investors(
                        [i.fk_investor for i in activity.involvements.all()]
                    ),
                }
            }
        else:
            investor_status_list = [
            ...
            query = {"terms": {"fk_status": investor_status_list}}
        sort = ["investor_identifier"]
        investors = self.execute_elasticsearch_query(
            query, doc_type="investor", fallback=False, sort=sort
        )
            ...
            def get_involvements(involvements):
                ...
                for involvement in involvements:
                    ...
                    if request.user.is_authenticated:
                        ...
                        involvement.fk_investor.fk_status_id
                        in InvestorBase.PUBLIC_STATUSES
                    ):
                        parents.append(involvement.id)
                return parents
        ...
        involvements = self.execute_elasticsearch_query(
            query, doc_type="involvement", fallback=False, sort=sort
        )
        results["involvements"] = self.filter_involvements(
            involvements, investors=investors
        )
        ...
        return getattr(self, "export_%s" % format)(
            self.get_data(results, format=format), "%s.%s" % (filename, format)
        )
    ...
    def export_csv(self, data, filename):
        ...
        deals_file = BytesIO()
        writer = csv.writer(
            deals_file, delimiter=";", encoding="utf-8"
        )  # encoding='cp1252'
        ...
        writer = csv.writer(
            involvements_file, delimiter=";", encoding="utf-8"
        )  # encoding='cp1252'
        ...
        writer = csv.writer(
            investors_file, delimiter=";", encoding="utf-8"
        )  # encoding='cp1252'
    ...
    def get_data(self, results, format=None):
        ...
        """
        data = {
            "deals": {"headers": [], "items": [], "max": {}},
            "involvements": {"headers": [], "items": []},
            "investors": {"headers": [], "items": []},
        }
        ...
        for form in DealUpdateView.FORMS:
            ...
            if formset_name:
                ...
                form_count = [
                    i.get("%s_count" % formset_name, 0) for i in results["deals"]
                ]
                ...
                for i in range(0, data["deals"]["max"][formset_name]):
                    for field_name, field in form.base_fields.items():
                        if field_name in exclude:
                            continue
                        if field_name.startswith("tg_") and not field_name.endswith(
                            "_comment"
                        ):
                            continue
                        headers.append(
                            "%s %i: %s" % (form.form_title, i + 1, str(field.label))
                        )
            else:
                for field_name, field in form.base_fields.items():
                    if field_name in exclude:
            ...
            if form.Meta.name == "investor_info":
                for field_name, field in ExportInvestorForm.base_fields.items():
                    if field_name in exclude:
                        continue
                    headers.append(
                        "%s: %s" % (_("Operating company"), str(field.label))
                    )
                    ...
                    for i in range(0, data["deals"]["max"][formset_name]):
                        for field_name, field in form.base_fields.items():
                            ...
                            if field_name.startswith("tg_") and not field_name.endswith(
                                ...
                                continue
                            row.append(
                                self.get_export_value(
                                    field_name, item, formset_index=i, format=format
                                )
                            )
                else:
                    for field_name, field in form.base_fields.items():
                        ...
                        if field_name.startswith("tg_") and not field_name.endswith(
                            ...
                            continue
                        row.append(
                            self.get_export_value(field_name, item, format=format)
                        )
                # Append operating company attributes to investor info
                if form.Meta.name == "investor_info":
                    exclude = []
                    ...
                    for field_name, field in ExportInvestorForm.base_fields.items():
                        ...
                        row.append(
                            self.get_export_value(
                                "operating_company_%s" % field_name, item, format=format
                            )
                        )
            rows.append(row)
