<script lang="ts">
  ...
  async function register() {
    const ret = await ($page.data.urqlClient as Client)
      .mutation<{ register: { ok: boolean; message: string } }>(
        gql`
          mutation Register(
            $username: String!
            $first_name: String!
    ...
    if (ret.data?.register?.ok) {
      ...
      register_failed_message = ret.data?.register?.message || ret.error.toString();
    }
  }
</script>
...
<div>
  ...
  {:else}
    <div class="mb-4 flex h-full w-full items-center justify-center">
      <div class="w-[clamp(300px,80%,900px)] rounded border-2 border-[#4b4b4b] p-6">
        {$_(
        ...
        <form
          class="my-6 grid gap-4 md:grid-cols-2"
